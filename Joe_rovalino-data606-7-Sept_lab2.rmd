...
output:
  html_document:
    css: /home/jr/Data606/Lab2/lab.css
    highlight: pygments
    theme: cerulean
...
source("/home/jr/Data606/Lab2/more/cdc.R")
```
...
There are 20000 cases in the dataset. There are 9 variables
genhlth : Factor and categorical;
exerany - Factor and categorical;
hlthplan - number and discrete;
smoke100 - number and discrete;
height  : number and discrete;
weight  : integer and discrete;
wtdesire: integer and discrete;
age     : integer and discrete;
gender  : Factor and categorical;
...
```{r table-exerany-prop, eval=TRUE}
table(cdc$exerany)/20000

```
The relative frequency for exeranyr is:
...
table(cdc$gender)
```
There are 9569 men is the population

```{r}
table(cdc$genhlth)
```
There are 4657 with excellent health. 
...
JR Answer:
More men have smoked 100 cigarettes than women. Men appear to smoke more then women. 
...
JR Answer
```{r under23andSmoke, eval=TRUE}
u_23andSmoke <- subset(cdc, age < 23  & smoke100 == 1)
```
...
```{r}
median(cdc$height)
quantile(cdc$height, 3/4)
quantile(cdc$height, 1/4)
```

Confirm that the median and upper and lower quartiles reported in the numerical 
summary match those in the graph. 

JR Answer:
67 is the median - matches the graph above
70 is the Q3 (75th) Quartile and matches the graph above
64 is the Q1 (25th) Quartile and matches the graph above

The purpose of a boxplot is to provide a 
...
JR Answer:
The looks like the median is lower based on regression of healthiness. i.e. 'excellent' health has a lower median then 'very good'. 'very good' health has a lower median then 'good' and etc. ;
The IRQ is smaller as are the whiskers (1.5 * Q#) as you regress from 'excellent' to ' poor'health. It appears as you get more unhealthy you tend to have a wider range to the data population (IQR gets bigger). 
...
The breaks create more bins of data to see slices of the data. It allows to see more peaks. 
...
```{r geomplot-weight-wtdesire, eval=TRUE}
library(ggplot2)
ggplot(cdc, aes(x = weight, y = wtdesire)) +
  geom_point(aes(color = weight), alpha = 0.4)
```
JR Answer:
Most people have a desire to have less weight then their current weight. 
...
-   Let's consider a new variable: the difference between desired weight 
    ...
    ```{r}
    library(dplyr)
    cdc <- mutate(cdc, wdiff = weight-wtdesire)
    structure(cdc)
...
JR Answer:
wdiff is an integer. If the observation is 0 then the person likes the exact weight they have. Since the formula was weight - wtdesire: positive # means the person desires to decrease real weight; a negative # means the person desires to increase real weight. 
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    ```{r hist-wdiff, eval=TRUE}
summary(cdc$wdiff)
    hist(cdc$wdiff)
    hist(log10(cdc$wdiff))

```
JR Answer:
Looking that histogram and especailly the log10 histogram - most people want to either slightly raise or lower their weight to the desired weight. Most people want to change weight.
...
#get a table of weight differnce by gender
table(cdc$wdiff, cdc$gender)
#create a dataframe with only male to get summary() for male
wdiff_m <- subset(cdc, gender =="m")
#wdiff_m
summary(wdiff_m$wdiff)
#create a dataframe subset with only femail to get summary() for femail
wdiff_f <- subset(cdc, gender == "f")
#wdiff_f
summary(wdiff_f$wdiff)
#create a log10 boxplot
boxplot(log10(cdc$wdiff) ~ cdc$gender)
```

JR Answer:
3150 of the 9569 men are happy with their weight as the difference is zero
2466 of the 10431 women are happy with their weight as the differnce is zero. 
A larger weighted ratio percentage is of men than women are happy with their weight (3150/9539 for men vs 2466/10431 for woman). The men who are not satified with their weight - the majority want to increase weight if you review the box plot.. While the women have both desired gainers and losers of weight. 
...
PSD
PSD2 <- mn1-sd1
PSD2 <-as.integer(PSD2)
PSD2
#str(cdc)
cdc2 <- subset(cdc, weight <= PSD & weight >= PSD2 )
str(cdc2)
cdc2$weight
